...
#### 📌 핵심 수식
사용자의 시급을 W, 소요 시간을 T(분), 직접 비용을 C라고 할 때 총 비용(TC)은 다음과 같습니다.
...
#### Primary
- **직장인/프리랜서**: 시급 개념이 명확하고, 시간 vs 돈 트레이드오프가 빈번한 사용자
- **의사결정 피로를 겪는 사람**: 선택지가 많아 비교 기준이 필요한 사용자

#### Secondary
- **학생**: 아르바이트 시급 기준으로 시간 가치를 학습하고자 하는 사용자
- **주부/육아맘**: 시간 절약형 소비(배달, 택시 등)의 실제 비용을 계산하고자 하는 사용자
...
- [TIME_ALLOCATION](docs/TIME_ALLOCATION.md): 시간 배분 및 우선순위 조정
- [CHANGELOG](docs/CHANGELOG.md): 변경 이력 (30개 버전, 6개 Phase)
- [TROUBLESHOOTING](docs/TROUBLESHOOTING.md): 문제 해결 과정 (17개 항목)
- [RETROSPECTIVE](docs/RETROSPECTIVE.md): 우선순위 조정 및 시간 배분 회고
...
### ✨ 주요 특징
1. **Total Cost Index**: 시간을 돈으로 환산하는 명확한 수식
2. **3단계 근거 시스템**: 결과 → 분해 → 상세 계산식
3. **실제 시나리오 프리셋**: 구체적 상황 기반 예시 (6개)
4. **세션 기반 히스토리**: 의사결정 세션 단위 관리
5. **이중 검증**: 프론트엔드 + 백엔드 안정성
6. **사용자 피드백 반영**: 4건 실제 피드백 100% 반영
...
print("README.md created successfully with UTF-8 encoding")
