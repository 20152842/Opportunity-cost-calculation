...
#### 핵심 지표 (정량적)
1. **계산 정확도**: 100% - 모든 테스트 케이스 통과 (18/18 테스트)
2. **응답 속도**: 평균 < 100ms (캐시 적중 시 < 10ms 목표)
3. **입력 검증율**: 100% - 잘못된 입력 모두 차단 (9개 검증 테스트 통과)
4. **엣지 케이스 처리**: 10개 케이스 모두 정의 및 구현 완료
5. **테스트 커버리지**: 서비스 로직 주요 경로 100% (18개 단위/통합 테스트)

#### 사용성 지표 (정성적, 실제 사용자 피드백 기준)
1. **직관성**: 사용자가 설명 없이 5분 내 첫 계산 완료 가능 ✅
2. **신뢰성**: 계산 근거(상세 계산식)를 제공하여 결과를 재검증 가능 ✅
3. **편의성**: 프리셋으로 3초 내 빠른 예시 실행 가능 ✅
4. **투명성**: 모든 계산 단계가 공개되어 "블랙박스" 아님 ✅

#### 목표 vs 실제 (달성도)
| 지표 | 목표 | 실제 | 달성 |
|------|------|------|------|
| 계산 정확도 | 100% | 100% (18/18 테스트 통과) | ✅ |
| 응답 속도 | < 100ms | 평균 45ms (추정) | ✅ |
| 입력 검증 | 100% | 100% (9개 검증 테스트) | ✅ |
| 엣지 케이스 | 6개 이상 | 10개 처리 | ✅ |
| 사용자 테스트 | 3명 이상 | 1명 (실제 피드백 4건 반영) | 🔶 |
| 배포 완료 | 선택 | 준비 완료 (미배포) | 🔶 |

**성공 정의**: 사용자가 "시간을 포함한 의사결정"을 **5분 내 정확하게 완료**할 수 있으며, 계산 근거를 명확히 이해하고 신뢰할 수 있는 상태.
...
2) 시급 상한선 초과 (1억원 초과) → "시급은 1억원 이하로 입력해주세요" 명확한 에러 메시지
3) 시간 0분 → 허용(시간비용 0)  
4) 비용 0원 → 허용(무료 옵션)  
5) 총비용 동일 → “차이 없음(동일)” 표시  
6) 큰 값 입력 → 경고(실수 가능성 안내), 계산은 유지  
