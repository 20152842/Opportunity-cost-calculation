...
- **조치 내용**
  - `showError()` 함수를 3단계로 개선:
    1. **`alert()` 팝업**으로 즉시 에러 내용 표시 → 사용자가 어디에 있든 즉시 인지
    2. 하단 에러 메시지 영역도 유지 (참고용, 복사 가능)
    3. `window.scrollTo({ top: 0, behavior: 'smooth' })`로 상단 자동 스크롤
- **결과**
  - 에러 발생 시 사용자 위치와 관계없이 팝업으로 즉시 알림.
  - 팝업 확인 후에도 하단 영역에서 에러 내용 재확인 가능.
- **배운 점**
  - 중요한 사용자 피드백(에러, 경고)은 **현재 뷰포트/스크롤 위치에 의존하지 않도록** 설계해야 한다.
  - 팝업(alert/confirm)은 사용자 흐름을 강제로 멈추지만, **필수 알림에는 가장 확실한 방법**이다.
  - 대안으로 상단 고정 토스트 알림을 구현할 수도 있지만, 과제 범위에서는 alert가 가장 빠르고 확실하다.
