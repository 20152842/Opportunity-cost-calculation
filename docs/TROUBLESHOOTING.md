...
## 18) 계산 결과 검증 부족 문제 (품질 개선)

- **증상**
  - 매우 큰 값 입력 시 계산 결과가 음수로 나오거나 비현실적인 값이 나와도 감지되지 않음.
  - 예: 시급 100만원, 시간 10,000분 → 총 비용이 오버플로우로 음수 표시.
- **원인**
  - 입력값 검증은 있지만, **계산 결과의 논리적 정확성** 검증이 없음.
  - Long 타입 오버플로우, 비현실적 비율 (시간 비용이 직접 비용의 100배 등) 미감지.
- **조치 내용**
  - **`validateCalculationResult()` 메서드 추가**:
    1. **오버플로우 체크**: 총 비용이 음수이거나 각 비용보다 작으면 에러
    2. **비현실적 비율 체크**: 시간 비용이 직접 비용의 100배 초과 시 경고 로그
    3. **비현실적 총액 체크**: 총 비용 10억원 초과 시 경고 로그
    4. **논리적 일관성 체크**: 총 비용 ≠ 직접 비용 + 시간 비용이면 에러
    5. **소수점 처리 체크**: 소요 시간이 0분 초과인데 시간 비용이 0원이면 경고
  - **입력 상한선 추가**:
    ```java
  ...
  - **테스트 케이스 11개 추가**:
    ...
    - 상한선 검증 테스트
- **결과**
  - 계산 오류를 사전에 감지하여 신뢰성 향상.
  - 사용자에게 명확한 에러 메시지 제공 ("계산 결과가 범위를 초과했습니다").
  - 로그를 통해 비현실적 입력 패턴 분석 가능 (단위 혼동 등).
- **커밋**: fdc041b
- **테스트 커버리지**: 14개 → 25개 (11개 추가)

---

## 19) 다안 비교 계산식이 간략하게 표시되던 문제
...
## 20) 다안 비교 → 2안 비교 전환 시 DOM 에러 발생 (치명적)
