...
| v1.4.1 | 2026-02-12 | 우선순위 조정 및 의사결정 기록 강화 | 실제 개발 과정의 trade-off, 막힌 부분, 시간 배분 변화를 명확히 기록 | RETROSPECTIVE.md 대폭 강화 (실패 사례 포함) | 03be125 |
...
| 버전 | 날짜 | 변경 요약 | 변경 이유 | 영향 | 커밋 |
|------|------|----------|----------|------|------|
| v1.8.0 | 2026-02-13 | 입력 검증 및 계산 결과 검증 강화 | ① 입력 상한선 미설정 시 오버플로우 위험 ② 계산 결과의 논리적 오류 검증 부재 | ⓐ @Max 검증 추가 (시간 7일, 비용 1억원) ⓑ validateCalculationResult 메서드 추가 (오버플로우/비율/일관성 체크) ⓒ 테스트 11개 추가 (총 25개) | fdc041b |
| v1.8.1 | 2026-02-13 | JavaScript 코드 스타일 통일 | 혼재된 함수 선언 방식(function vs 화살표 함수)으로 인한 일관성 문제 | 모든 함수를 화살표 함수로 통일, 코드 일관성 100% 달성 | dc5a85f |
| v1.9.0 | 2026-02-13 | 모드 전환 및 계산식 표시 버그 수정 | ① 다안 비교 계산식이 너무 간단함 ② 다안→2안 전환 시 DOM 에러 발생 | 상세 계산식 자동 생성, HTML 구조 자동 복원, 모드 전환 시 결과 숨김 | a1a9a08 |
| v1.9.1 | 2026-02-13 | Railway 배포 완료 | 실제 동작하는 웹앱 URL 제공으로 접근성 향상 | 배포 URL: https://opportunity-cost-calculation-production.up.railway.app/ | 8023563 |
